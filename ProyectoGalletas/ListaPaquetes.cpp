...
void ListaPaquetes::insertar(int cantidadGalletasPorPaquete,QString _nombre,int _cantidadPaquetes,int procesoPaquetes,int duracionPaquetes,int probabilidad){
    if (this->primerNodo == NULL){
        ...
        this->listaTransportadores->insertarTransportador(_nombre,2,2,0);//(int _maximo,int _tiempoTransporte,int _actuales,QString _tipoPaquete
    }else{
        ...
        if (buscado!=NULL){
            ...
            this->listaTransportadores->insertarTransportador(_nombre,2,2,0);
        }
    }
    this->largo++;
